            ...
            >
                @foreach ($this->visibleColumns as $column)
                    <div
                        ...
                        @class([
                            'font-semibold bg-pg-primary-100 ' => data_get($column, 'hidden'),
                            'py-1' => $loop->first || $loop->last,
                            'cursor-pointer text-sm flex justify-between block px-4 py-2 text-pg-primary-800 hover:bg-pg-primary-100 hover:text-black-300'
                        ])
                    >
                        <div>
                        ...
                        @if (!data_get($column, 'hidden'))
                            <x-livewire-powergrid::icons.eye class="text-pg-primary-200" />
                        @else
