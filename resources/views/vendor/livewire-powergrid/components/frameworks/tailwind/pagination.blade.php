        ...
        @if ($recordCount === 'full')
            <div @class(['mr-3' => $paginator->hasPages()])>
                <div @class([
                    ...
                    'leading-5 text-center text-pg-primary-700 text-md sm:text-right',
                ])>
            ...
            >
                <div class="flex justify-center mt-2 md:flex-none md:justify-end sm:mt-0">
                    ...
                    @if (!$paginator->onFirstPage())
                        ...
                        <a
                            class="cursor-pointer relative inline-flex items-center px-2 py-2 text-sm font-medium text-pg-primary-500 bg-white border border-pg-primary-300 leading-5 hover:text-pg-primary-400 focus:z-10 focus:outline-none focus:shadow-outline-blue active:bg-pg-primary-100 active:text-pg-primary-500 transition ease-in-out duration-150"
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span
                                        class="select-none relative z-10 inline-flex items-center px-3 py-2 -ml-px text-sm font-bold text-primary-700 bg-primary-100 border border-primary-300 cursor-default select-none"
                                    >{{ $page }}</span>
                                @elseif (
                            ...
                            @else
                                @if (method_exists($paginator, 'getCursorName'))
                                    <button
                                        ...
                                        class="select-none p-2 m-1 text-center text-white bg-pg-primary-600 border-pg-primary-400 rounded cursor-pointer border-1 hover:bg-pg-primary-600 hover:border-pg-primary-800"
                                    >
                                        <svg
                                            fill="none"
                                            viewBox="0 0 24 24"
